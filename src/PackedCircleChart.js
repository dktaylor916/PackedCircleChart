...
on('mouseover', function(d) {
  ...
  d3.selectAll("."+selectedClass).selectAll('.help').style("opacity",1)
  d3.selectAll("."+selectedClass).selectAll('.pct').style("opacity",1)
  d3.selectAll("."+selectedClass).select('circle.node').style("opacity",1).style('stroke','black').style('stroke-width',1)
  
  d3.selectAll("*:not(."+selectedClass+")").select('.help').style("opacity",0)
  d3.selectAll("*:not(."+selectedClass+")").select('.pct').style("opacity",0)
  d3.selectAll("*:not(."+selectedClass+")").select('circle.node').style("opacity",.5)
  
  d3.selectAll('.legendDots').filter(function(d){ return d == selectedColor}).style('opacity',1)
  d3.selectAll('.legendDots').filter(function(d){ return d != selectedColor}).style('opacity',.2)

  d3.selectAll('.legendText').filter(function(d){ return d == selectedText}).style('opacity',1)
  d3.selectAll('.legendText').filter(function(d){ return d != selectedText}).style('opacity',.2)
...
.on('mouseout', function() {
  d3.selectAll('.legendDots').style('opacity',1)
  d3.selectAll('.legendText').style('opacity',1)
  d3.select(this).classed('selected',false)
  d3.selectAll('.help').style('opacity',function(d){
    if (d.data.mean < 5){
      return 0
    }
    else{return 1}
  })
  d3.selectAll('.pct').style('opacity',function(d){
    if (d.data.mean < 5){
      return 0
    }
    else{return 1}
...
d3.select('#demo1').append('text').attr('x',10*2).attr('y',280*2).attr('font-size',3.5*2).attr('fill','#666666').text('Source: DHS 2005-19, most recent survey per country. Note: % is of ever-partnered women aged 15-49 that have experienced physical or sexual violence. Average taken')
d3.select('#demo1').append('text').attr('x',10*2).attr('y',285*2).attr('font-size',3.5*2).attr('fill','#666666').text('across 51 countries with data available.')
